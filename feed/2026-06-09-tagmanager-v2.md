...
The `VariableFormatValue` schema has been updated to include a new `automatic` enum value for the `convertToNumber` property. This addition allows the API to handle numeric conversion of strings by automatically detecting the decimal separator, reducing the need for custom logic when dealing with internationalized number formats (e.g., dot vs. comma separators).
